...
GitHub Actions คือแพลตฟอร์ม CI/CD ในตัวของ GitHub ทำหน้าที่เรียกใช้ชุดคำสั่งอัตโนมัติเมื่อเกิด Event ในระบบ
...
<!--
Notes:
- เน้นย้ำโครงสร้างไดเรกทอรีของเวิร์กโฟลว์ว่าต้องเก็บไว้ที่ `.github/workflows/` เสมอ ไม่เช่นนั้น GitHub จะตรวจไม่พบ
- อธิบายส่วนหลักของ YAML: `on` (ตัวกระตุ้นให้รัน เช่น push/PR), `jobs` (งานหลักที่จะทำ), และ `steps` (ขั้นตอนย่อยในแต่ละงาน)
-->
...
## 8. ภาคปฏิบัติ Lab 2.2: Production CI/CD Pipeline
การสร้างเวิร์กโฟลว์ระดับมืออาชีพแบบ Multi-Job, ตรวจสอบ Lint, สแกนช่องโหว่ความปลอดภัย และเก็บ Artifacts

1. **เตรียม ESLint & Scripts**:
   * ติดตั้ง `eslint` กำหนดสคริปต์ `"lint"` และ `"build"` ใน `package.json`
2. **สร้าง Multi-Job Workflow**:
   * Job 1: `security-and-lint` ➔ ตรวจสอบโค้ดด้วย Lint และรัน `npm audit --audit-level=high`
   * Job 2: `build-and-test` ➔ รันเทสต์ Jest, สั่ง `npm run build` และต้องการ Job 1 ให้ผ่านก่อน (`needs`)
3. **ส่งออก Build Artifacts**:
   * ใช้ `actions/upload-artifact@v4` อัปโหลดโฟลเดอร์ผลลัพธ์ (`dist/`) เพื่อส่งต่อให้ทีม CD นำไป Deploy หรือจัดเก็บไฟล์ย้อนหลัง

<!--
Notes:
- อธิบายแนวคิด "Fail Fast" ในการสร้าง pipeline: ยิ่งรู้ตัวว่าโค้ดผิดพลาดเร็ว ยิ่งแก้ไขได้เร็วและประหยัดค่าใช้จ่ายคอมพิวเตอร์
- ชี้ให้เห็นคำสั่ง `needs: security-and-lint` ใน YAML ว่าถ้าจ็อบแรก (คุณภาพโค้ด/ความปลอดภัย) ตก จ็อบสองจะไม่ทำงานเลย
- แนะนำผู้เรียนเกี่ยวกับการใช้ `actions/upload-artifact@v4` ว่าเป็นขั้นตอนเตรียมไฟล์สำหรับการทำ Continuous Delivery (CD) เพื่อนำไปใช้งานหรือส่งต่อไปติดตั้งบนระบบคลาวด์/เซิร์ฟเวอร์จริง
-->
