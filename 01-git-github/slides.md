...
<!--
Notes:
- จุดประสงค์ของแล็บนี้คือช่วยให้นักพัฒนาคุ้นเคยกับการแก้ไข Merge Conflict ที่พบบ่อยเมื่อทำงานร่วมกันในทีม
- อธิบายสัญลักษณ์ <<<<<<<, =======, >>>>>>> ว่าฝั่งไหนคือโค้ดในเครื่องปัจจุบัน (HEAD) และฝั่งไหนคือโค้ดบนกิ่งที่ดึงเข้ามารวม (feat-login)
- เน้นย้ำให้ลบตัวบ่งชี้ทับซ้อนเหล่านี้ออกให้หมดก่อนรัน git add / commit เสมอ
-->

---

## 10. ภาคปฏิบัติ Lab 1.2: Git Rebase & PR Template
การทำงานระดับ Enterprise ด้วยการทำ Rebase และการกำหนดเทมเพลตรีวิวโค้ด

1. **สร้าง PR Template**:
   * สร้างไฟล์ `.github/pull_request_template.md` กำหนดหัวข้อ Checklist เพื่อรีวิวโค้ดก่อน Commit
2. **แก้ไขทับซ้อนและ Rebase**:
   * แยกกิ่ง `feat/payment` แก้ไขไฟล์ `app.js` และ Commit
   * สลับไปกิ่ง `main` แก้ไขบรรทัดเดียวกัน และ Commit เพื่อสร้าง Conflict
   * สลับกลับมาที่กิ่งฟีเจอร์แล้วรัน `git rebase main`
3. **แก้ไข Conflict และผลักโค้ดขึ้นระบบ**:
   * แก้ไขจุดที่ทับซ้อนกันใน Text Editor
   * รัน `git add app.js` และ `git rebase --continue`
   * ผลักขึ้น GitHub ปลอดภัยด้วย `git push origin feat/payment --force-with-lease`

<!--
Notes:
- อธิบายผู้เรียนว่า Rebase ต่างจาก Merge อย่างไร: Merge จะสร้าง commit ใหม่เชื่อมสองฝั่ง แต่ Rebase จะดึง commit ของเราออกมา แล้วเอาไปต่อท้าย commit ล่าสุดของ main (ช่วยให้ประวัติเป็นเส้นตรงไม่รก)
- ย้ำความสำคัญของ PR Template ว่าช่วยควบคุมมาตรฐานการทำงานในองค์กร เช่น การเตือนให้เขียนวิธีทดสอบและตรวจสอบ Lint/Console logs เสมอ
- ชี้ให้เห็นว่าการใช้ --force-with-lease ปลอดภัยกว่า -f เพราะจะปฏิเสธการเซฟทับหากมีคนอื่น push โค้ดใหม่ขึ้นไปบนเซิร์ฟเวอร์ที่เรายังไม่ได้ดึงลงมา
-->
